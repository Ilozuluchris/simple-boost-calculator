...
w3 = Web3(Web3.HTTPProvider(f'https://mainnet.infura.io/v3/'
                            f'{os.environ["INFURA_PROJECT_ID"]}'))
...
vault_contract = w3.eth.contract(address=vault_contract_address,
                                 abi=vault_abi)
...
boost_contract = w3.eth.contract(address=boost_contract_address,
...
token_locker_contract = w3.eth.contract(address=token_locker_address,
...
def get_account_weight(claimant, week):
    ...
    return int(token_locker_contract.functions
               .getAccountWeightAt(claimant, week).call())
...
def get_emission_for_week(week_to_find):
    ...
    # Three years of emissions +  4 weeks of high prisma reward
    weeks_to_calculate = 156 + 4

    emissions_data, remaining_prisma = \
        calculate_remaining_prisma(total_prisma_available, weeks_to_calculate)
...
def boost_calc(claimant, weeks_to_be_locked, amount_to_be_locked):
    ...
    return boost_calc_from_contract(claimant, 0, total_emissions,
                                    system_week, weeks_to_be_locked,
                                    amount_to_be_locked)


def boost_calc_from_contract(claimant, previous_amount, total_weekly_emissions,
                             system_week, lock_weeks, lock_amount):
    ...
    account_weight, total_weight = \
        get_weights_to_be_used(claimant, current_week, lock_weeks, lock_amount)
    ...
    max_boosted = 0 if previous_amount >= max_boostable \
        else max_boostable - previous_amount
    boosted = 0 if previous_amount >= full_decay \
        else full_decay - max_boostable

    return max_boosted, boosted


def get_weights_to_be_used(claimant, current_week,
                           weeks_to_be_locked, amount_to_be_locked):
